...
/// 网络请求相关配置
@property (nonatomic, strong) YZNetworkConfig *config;
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, YZBaseRequest *> *requestsRecordDictM;
@end
...
+ (YZNetworkManager *)sharedManager {
    static YZNetworkManager *_instance = nil;
...
/// 根据请求对象返回http请求编码器
/// @return http请求编码器
- (AFHTTPRequestSerializer *)getRequestSerializerForRequest:(YZBaseRequest *)request {
    AFHTTPRequestSerializer *requestSerializer = nil;
    YZRequestSerializerType serializerType = request.requestSerializerType;
    switch (serializerType) {
        case YZRequestSerializerTypeJSON:
...
- (NSURLSessionTask *)sessionTaskForRequest:(YZBaseRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
    
    // 请求方式
    YZRequestMethod method = [request requestMethod];
    // 请求地址
...
- (void)cancelAllRequests {
    ...
    if (allKeys != nil && allKeys.count > 0) {
        ...
        for (NSNumber *key in copiedKeys) {
            Lock();
            YZBaseRequest *request = self.requestsRecordDictM[key];
            Unlock();
...
/// 请求成功
/// @param request 请求对象
- (void)requestDidSucceedWithRequest:(YZBaseRequest *)request {
...
/// 请求失败
/// @param request 请求对象
- (void)requestDidFailWithRequest:(YZBaseRequest *)request error:(NSError *)error {
    request.error = error;
...
- (void)handleRequestResult:(NSURLSessionTask *)task responseObject:(id)responseObject error:(NSError *)error {
    ...
    Lock();
    YZBaseRequest *request = [self.requestsRecordDictM objectForKey:@(task.taskIdentifier)];
    Unlock();
    ...
    if ([responseObject isKindOfClass:[NSData class]]) {
        request.responseString = [[NSString alloc] initWithData:responseObject encoding:[YZNetworkUtils stringEncodingWithRequest:request]];

        switch (request.responseSerializerType) {
            case YZResponseSerializerTypeHTTP:
                ...
                break;
            case YZResponseSerializerTypeJSON:
                ...
                break;
            case YZResponseSerializerTypeXMLParser:
...
- (BOOL)validateResult:(YZBaseRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
// TODO data validate
    return YES;
}
...
- (YZNetworkConfig *)config {
    if (_config == nil) {
        _config = [YZNetworkConfig sharedConfig];
    }
...
- (NSMutableDictionary <NSNumber *, YZBaseRequest *> *)requestsRecordDictM {
